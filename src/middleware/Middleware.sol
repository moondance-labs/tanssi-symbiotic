...
contract Middleware is SimpleKeyRegistry32, Ownable {
    ...
    function quickSort(ValidatorData[] memory arr, int256 left, int256 right) public pure {
        int256 i = left;
        int256 j = right;
        if (i == j) return;
    ...
    /**
     * @dev Sorts operators by their total stake in descending order, after 500 it will be almost impossible to be used on-chain since 500 ≈ 31M gas
     * @param epoch The epoch number
     * @return sortedValidatorData Array of sorted validator data
     */
    function sortOperatorsByVaults(
        uint48 epoch
    ) external view returns (ValidatorData[] memory sortedValidatorData) {
        ValidatorData[] memory validatorSet = getValidatorSet(epoch);
        quickSort(validatorSet, 0, int256(validatorSet.length - 1));
        sortedValidatorData = validatorSet;
    }
